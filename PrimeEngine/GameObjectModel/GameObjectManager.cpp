	...
	if (!haveObject)
	{
		...
		else
		{
			if (pRealEvent->hasCustomOrientation)
			{
				...
				// A4: hard code physics component to car and floor
				if (StringOps::strcmp(pRealEvent->m_meshFilename, "nazicar.x_carmesh_mesh.mesha") == 0) {
					Handle hPhysicsComponent("PHYSICS_COMPONENT", sizeof(PhysicsComponent));
					PhysicsComponent* pPC = new(hPhysicsComponent) PhysicsComponent(*m_pContext, m_arena,
						hPhysicsComponent, 
						Vector3(-2.03452384, 0, -2.77164459), 
						Vector3(2.03452384, 1.68993318, 3.87164483),
						hSN, true);
					pPC->addDefaultComponents();

					pSN->m_hPC = hPhysicsComponent;
				}

				if (StringOps::strcmp(pRealEvent->m_meshFilename, "cobbleplane.x_pplaneshape1_mesh.mesha") == 0) {
					Handle hPhysicsComponent("PHYSICS_COMPONENT", sizeof(PhysicsComponent));
					PhysicsComponent* pPC = new(hPhysicsComponent) PhysicsComponent(*m_pContext, m_arena,
						hPhysicsComponent,
						Vector3(-12.5, -2.0f, -12.5),
						Vector3(12.5, -0.5f, 12.5),
						hSN, true);
					pPC->addDefaultComponents();
	...
	else
	{
		...
		if (pRealEvent->hasCustomOrientation)
		{
			...
			if (hSN.isValid())
			{
				...
				//	Handle hPhysicsComponent("PHYSICS_COMPONENT", sizeof(PhysicsComponent));
				//	PhysicsComponent* pPC = new(hPhysicsComponent) PhysicsComponent(*m_pContext, m_arena,
				//		hPhysicsComponent, Vector3(0, 1, 0), Vector3(2, 2, 5), hSN, true);
				//	pPC->addDefaultComponents();
