...
namespace PE {
    ...
    void EffectManager::loadDefaultEffects()
    {
		...
		// ms1 pGlowMultipleTargetTextureGPU created
		m_hGlowMultipleTargetTextureGPU = Handle("TEXTURE_GPU", sizeof(TextureGPU));
		TextureGPU* pGlowMultipleTargetTextureGPU = new(m_hGlowMultipleTargetTextureGPU) TextureGPU(*m_pContext, m_arena);
		...
		// ms1 pGlowMultipleTargetTextureGPU init
		pGlowMultipleTargetTextureGPU->createMultipleDrawableIntoColorTextureWithDepth(m_pContext->getGPUScreen()->getWidth(), m_pContext->getGPUScreen()->getHeight(), SamplerState_NoMips_MinTexelLerp_NoMagTexelLerp_Clamp);
		...
		m_2ndGlowTargetTextureGPU.createDrawableIntoColorTexture(m_pContext->getGPUScreen()->getWidth() , m_pContext->getGPUScreen()->getHeight() , SamplerState_NoMips_NoMinTexelLerp_MagTexelLerp_Clamp);
