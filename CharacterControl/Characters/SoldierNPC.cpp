...
SoldierNPC::SoldierNPC(PE::GameContext &context, PE::MemoryArena arena, PE::Handle hMyself, Event_CreateSoldierNPC *pEvt) : Component(context, arena, hMyself)
{
	...
	for (int iSkin = 0; iSkin < numskins; ++iSkin)
	{
		...
		//PhysicsComponent* pPC = new(hPhysicsComponent) PhysicsComponent(*m_pContext, m_arena, hPhysicsComponent,hMeshInstance,hSN);
		PhysicsComponent* pPC = new(hPhysicsComponent) PhysicsComponent(*m_pContext, m_arena, 
			hPhysicsComponent,Vector3(-0.45f,0.f,-0.3f), Vector3(0.45f,1.82f,0.3f) , hSN,false);
		pPC->addDefaultComponents();

		pMainSN->m_hPC = hPhysicsComponent;
