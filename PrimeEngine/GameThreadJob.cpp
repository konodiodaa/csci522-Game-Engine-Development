...
int ClientGame::runGameFrame()
{
    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
			...
			|| Event_GATHER_DRAWCALLS_Z_ONLY::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			if (!zOnly)
			{
				...
				//float pMin[] = { 1.0f,1.0f,1.0f };
				//float pMax[] = { 5.0f,5.0f,5.0f };
        ...
        else if (Event_FLY_CAMERA::GetClassId() == pGeneralEvt->getClassId()) // used to check the event in the event queue.
        {
            Event_FLY_CAMERA *pRealEvent = (Event_FLY_CAMERA *)(pGeneralEvt); // create the event with the resource in event queue->pGeneralEvt
	...
	if (!m_runGame)
	{
		...
		// this is the last iteration of game thread, wait for all other threads to be done with current frame
			...
			g_physicsThreadLock.lock(); // wait till previous draw is finished
			g_physicsThreadShouldExit = true;

			g_physicsThreadLock.unlock(); // release render thread lock
			// wake up render thread
			g_physicsCanStartCV.signal();

			while (!g_physicsThreadExited)
			{
