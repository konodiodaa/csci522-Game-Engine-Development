...
// ms1 multiple texture creation
#if 1
void TextureGPU::createMultipleDrawableIntoColorTextureWithDepth(PrimitiveTypes::UInt32 w, PrimitiveTypes::UInt32 h, ESamplerState sampler, bool use32BitRedForDepth /* = false*/)
{
	StringOps::writeToString("createDrawableIntoColorTextureWithDepth", m_name, 256);
	m_samplerState = sampler;
...
//glFramebufferParameteri(GL_FRAMEBUFFER, GL_FRAMEBUFFER_DEFAULT_WIDTH, w); / no need for this, since it take size of the texture passed
	...
	// ------------------------------------------------------------------------------------------------
	glGenTextures(1, &m_deferredTexture_pos);
	glBindTexture(GL_TEXTURE_2D, m_deferredTexture_pos);
	IRenderer::checkForErrors("glTexImage2D");
#if PE_PLAT_IS_IOS
	if (use32BitRedForDepth)
		PEWARN("We are creating depth+color and storing dpeth in color. for ios we store it as rgba, since we cant store it as 32bit red..");
	...
	glTexImage2D(
		...
		use32BitRedForDepth ? GL_RGBA : GL_RGBA,				// InternalFormat
#else
		use32BitRedForDepth ? GL_R32F : GL_RGBA16F,						// InternalFormat
#endif
	...
	glFramebufferTexture2DOES(GL_FRAMEBUFFER_OES, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, m_texture, 0);
#else
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, m_deferredTexture_pos, 0);
#endif


// create normal  buffer
// ------------------------------------------------------------------------------------------------
	glGenTextures(1, &m_deferredTexture_normal);
	glBindTexture(GL_TEXTURE_2D, m_deferredTexture_normal);
	IRenderer::checkForErrors("glTexImage2D");
#if PE_PLAT_IS_IOS
	if (use32BitRedForDepth)
		PEWARN("We are creating depth+color and storing dpeth in color. for ios we store it as rgba, since we cant store it as 32bit red..");
	...
	glFramebufferTexture2DOES(GL_FRAMEBUFFER_OES, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_2D, m_texture, 0);
#else
	glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, m_deferredTexture_normal, 0);
#endif
