...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
    ...
    if ( false && pMeshCaller->m_performBoundingVolumeCulling)
    {
        ...
        for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
        {
			...
			Vector3 bbox_min(pMeshCaller->m_bbx[0][0], pMeshCaller->m_bbx[0][1], pMeshCaller->m_bbx[0][2]);
			Vector3 bbox_max(pMeshCaller->m_bbx[1][0], pMeshCaller->m_bbx[1][1], pMeshCaller->m_bbx[1][2]);

			bbox_min = pSN->m_worldTransform * bbox_min;
			bbox_max = pSN->m_worldTransform * bbox_max;

			float pMin[3] = { bbox_min.m_x,bbox_min.m_y,bbox_min.m_z };
			float pMax[3] = { bbox_max.m_x,bbox_max.m_y,bbox_max.m_z };

			DebugRenderer::Instance()->createAABB(pMin, pMax);

			bool isNotCulled = pcam->FrustumCulling(bbox_min,bbox_max);
