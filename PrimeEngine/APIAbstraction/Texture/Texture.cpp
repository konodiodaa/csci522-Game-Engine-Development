...
void TextureGPU::createDeferredTextureNoFamily(const char* textureFilename, const char* package)
{
	PEString::generatePathname(*m_pContext, textureFilename, package, "Textures", PEString::s_buf, PEString::BUF_SIZE);
	...
	*/
	...
	PEASSERT(SUCCEEDED(hr), "Creating shader resource view");
#elif APIABSTRACTION_OGL
	m_deferredTexture_pos = gfxLoadDDSTexture(PEString::s_buf);
#elif PE_PLAT_IS_PSVITA
	m_texture = gfxLoadDDSTexture(PEString::s_buf);
#	endif
}
...
void TextureGPU::createDeferredTextureGPU(const char* textureFilename, const char* package, ESamplerState samplerState /*= = SamplerState_Count*/)
{
	//default
	m_samplerState = SamplerState_MipLerp_MinTexelLerp_MagTexelLerp_Wrap;
	...
	createDeferredTextureNoFamily(textureFilename, package);
	m_family = TextureFamily::COLOR_MAP;
