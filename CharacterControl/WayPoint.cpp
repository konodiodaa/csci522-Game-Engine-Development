...
int Event_CREATE_WAYPOINT::l_Construct(lua_State* luaVM)
{
	...
	const char* nextWayPointName3 = lua_tostring(luaVM, -numArgs--);

	int needToRun = (int)lua_tonumber(luaVM, -numArgs--);

	pEvt->m_needToRunToThisPoint = needToRun;
	...
	StringOps::writeToString(nextWayPointName1, pEvt->m_nextWaypointName1, 32);
	StringOps::writeToString(nextWayPointName2, pEvt->m_nextWaypointName2, 32);
	StringOps::writeToString(nextWayPointName3, pEvt->m_nextWaypointName3, 32);
...
: Component(context, arena, hMyself)
{
	...
	StringOps::writeToString(pEvt->m_nextWaypointName1, m_nextWayPointName1, 32);
	StringOps::writeToString(pEvt->m_nextWaypointName2, m_nextWayPointName2, 32);
	StringOps::writeToString(pEvt->m_nextWaypointName3, m_nextWayPointName3, 32);

	m_needToRunToThisWayPoint = pEvt->m_needToRunToThisPoint;
