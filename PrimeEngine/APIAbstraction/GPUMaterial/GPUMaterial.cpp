...
void GPUMaterial::createFromMaterialCPU(MaterialCPU &matcpu)
{
	...
	for (PrimitiveTypes::UInt32 iTex = 0; iTex <  matcpu.m_textureFilenames.m_size; iTex++)
	{
		...
		else
		{
			...
			{
				// ms1 change to create deferred textureGPU
				...
				//htgpu = GPUTextureManager::Instance()->createDeferredTextureGPU(texFilename.m_data.getFirstPtr(), texPackage.m_data.getFirstPtr(), customSamplerState);
